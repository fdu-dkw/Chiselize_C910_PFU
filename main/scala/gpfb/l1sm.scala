...
class l1sm (PA_WIDTH:Int) extends RawModule {
  ...
  //l1_pf_ppn clk
  wire.entry_l1_pf_ppn_clk_en := wire.entry_l1_pf_addr_init_vld && wire.pfu_dcache_pref_en || wire.entry_l1_pf_ppn_up_vld
  //例化gated_clk
  ...
  withClockAndReset(wire.entry_l1_pf_ppn_clk,(!io.cpurst_b.asBool).asAsyncReset){
    ...
    when(!io.cpurst_b.asBool){
    ...
    }.elsewhen(wire.entry_l1_pf_addr_init_vld &&  wire.pfu_dcache_pref_en){
      ...
      entry_l1_page_sec              := wire.pfu_get_page_sec
  ...
  withClockAndReset(io.entry_clk,(!io.cpurst_b.asBool).asAsyncReset){
    ...
    }.elsewhen(wire.entry_l1_pf_va_add_vld || wire.entry_pf_inst_vld && io.entry_l1sm_va_can_cmp){
  ...
  withClockAndReset(io.entry_clk, (!io.cpurst_b.asBool).asAsyncReset) { //注意asAsyncReset
    val entry_in_l1_pf_region = RegInit(reg.entry_in_l1_pf_region);
...
*/
  ...
  val entry_l1_state = withClockAndReset(io.entry_clk.asBool.asClock, (!io.cpurst_b.asBool).asAsyncReset) {
    RegInit(UInt(3.W), args.L1_INIT_PF_ADDR)
...
//state1
  ...
  wire.entry_l1_pf_va_add_strideh(PA_WIDTH-1,0)  := io.entry_l1_pf_va(PA_WIDTH-1,0) + wire.entry_strideh(PA_WIDTH-1,0)
  wire.entry_l1_pf_va_sum_4k(12,0)  := Cat("b0".U(1.W),io.entry_l1_pf_va(11,0)) + wire.entry_strideh(12,0)
