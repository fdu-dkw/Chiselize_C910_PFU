Package pfbTOP   包pfbTOP

import   进口 chisel3.   进口chisel3.__
import   进口 chisel3.util.   进口chisel3.util.__

class   类 类pfbIO扩展Bundle {pfbIO extends   扩展 Bundle {
  val   瓦尔 amr_wa_cancel =输入(Bool())val   瓦尔 amr_wa_cancel = Input(Bool())
  val   瓦尔 cp0_lsu_icg_en = Input(Bool())
  val   瓦尔 cp0_lsu_l2_st_pref_en = Input(Bool())
  val   瓦尔 cp0_lsu_pfu_mmu_dis = Input(Bool())
  val   瓦尔 cp0_yy_clk_en = Input(Bool())
  val   瓦尔 cp0_yy_priv_mode = Input(UInt(2.W))
  val   瓦尔 cpurst_b = Input(Bool())
  val   瓦尔 ld_da_ldfifo_pc =输入(UInt(15.W))val   瓦尔 ld_da_ldfifo_pc = Input(UInt(15.W))
  val   瓦尔 ld_da_page_sec_ff = Input(Bool())
  val   瓦尔 ld_da_page_share_ff = Input(Bool())
  val   瓦尔 ld_da_pfu_act_dp_vld = Input(Bool())
  val   瓦尔 ld_da_pfu_act_vld =输入(Bool())val   瓦尔 ld_da_pfu_act_vld = Input(Bool())
  val   瓦尔 ld_da_pfu_evict_cnt_vld = Input(Bool())
  val   瓦尔 ld_da_pfu_pf_inst_vld =输入(Bool())val   瓦尔 ld_da_pfu_pf_inst_vld = Input(Bool())
  val   瓦尔 ld_da_ppfu_va =输入(UInt(40w))val   瓦尔 ld_da_ppfu_va = Input(UInt(40.W))
  val   瓦尔 ld_da_ppn_ff = Input(UInt(28 w)) val   瓦尔 ld_da_ppn_ff =输入(UInt(28 w))val   瓦尔 ld_da_ppn_ff = Input(UInt(28.W))val   瓦尔 ld_da_ppn_ff =输入(UInt(28 w))
  val   瓦尔 lsu_pfu_l1_dist_sel = Input(UInt(4.W))val   瓦尔 lsu_pfu_l1_dist_sel =输入(UInt(4.W))
  val   瓦尔 lsu_pfu_l2_dist_sel = Input(UInt(4.W))val   瓦尔 lsu_pfu_l2_dist_sel =输入(UInt(4.W))
  val   瓦尔 lsu_special_clk = Input(Bool())
  val   瓦尔 pad_yy_icg_scan_en = Input(Bool())
  val   瓦尔 pfb_no_req_cnt_val = Input(int (6.W))val   瓦尔 pfb_no_req_cnt_val = Input(UInt(6.W))
  val   瓦尔 pfb_timeout_cnt_val =输入(UInt(8 w))val   瓦尔 pfb_timeout_cnt_val = Input(UInt(8.W))
  pfu_biu_pe_req_sel_l1 = Input(Bool())val   瓦尔 pfu_biu_pe_req_sel_l1 = Input(Bool())
  Val pfu_dcache_pref_en = Input(Bool())val   瓦尔 pfu_dcache_pref_en = Input(Bool())
  val   瓦尔 pfu_get_page_sec = Input(Bool())
  val   瓦尔 pfu_get_page_share =输入(Bool())val   瓦尔 pfu_get_page_share = Input(Bool())
  val   瓦尔 pfu_get_ppn = Input(UInt(28 w)) val   瓦尔 pfu_get_ppn =输入(UInt(28 w))val   瓦尔 pfu_get_ppn = Input(UInt(28.W))val   瓦尔 pfu_get_ppn =输入(UInt(28 w))
  val   瓦尔 pfu_get_ppn_err = Input(Bool())
  val   瓦尔 pfu_get_ppn_vld =输入(Bool())val   瓦尔 pfu_get_ppn_vld = Input(Bool())
  val   瓦尔 pfu_l2_pref_en = Input(Bool())
  val pfu_mmu_pe_req_sel_l1 = Input(Bool())
  val pfu_pfb_create_pc = Input(UInt(15.W))val   瓦尔 pfu_pfb_create_pc =输入(UInt(15 w))
  val pfu_pfb_create_stride = Input(UInt(11.W))
  val pfu_pfb_create_stride_neg = Input(Bool())
  val pfu_pfb_create_strideh_6to0 = Input(UInt(7.W))val   瓦尔 pfu_pfb_create_strideh_6to0 = Input(UInt(7 w))
  val pfu_pfb_create_type_ld = Input(Bool())val   瓦尔 pfu_pfb_create_type_ld =输入(Bool())
